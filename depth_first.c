...
struct node * move_block(struct node *, int , int , int );
...
int state_doesnt_exist(struct node *, struct node *, struct node *,
                       int, int);
...
/* RULE 1 */   if(is_uncovered(n, i))
                {
                  ...
                  if(n->blocks[i] != TABLE)
                  {
/* RULE 3 */         if(state_doesnt_exist(OPEN, CLOSED, n, i, TABLE))
                      {
                        new = move_block(n, node_number, i, TABLE);
                        if(is_a_goal_node(new, goal))
                        {
                           solution_found(new, &searching);
                        }
                        node_number++;
...
/* RULE 2 */            if(is_uncovered(n, j))
                         {
/* RULE 3 */               if(state_doesnt_exist(OPEN, CLOSED, n, i,j))
                             {
                              new = move_block(n, node_number, i, j);
                              if(is_a_goal_node(new, goal))
                              {
                                 solution_found(new, &searching);
                              }
